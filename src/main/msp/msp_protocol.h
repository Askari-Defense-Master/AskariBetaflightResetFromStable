...
// Protocol numbers used both by the wire format, config system, and field
// setters.
...
// #define BETAFLIGHT_IDENTIFIER "BTFL" Actual value stored in
...
#define BOARD_IDENTIFIER_LENGTH                                                \
  4 // 4 UPPER CASE alpha numeric characters that identify the board being used.
...
#define MSP_BOARD_ALIGNMENT_CONFIG                                             \
  38 // out message: Get board alignment configuration
#define MSP_SET_BOARD_ALIGNMENT_CONFIG                                         \
  39 // in message:  Set board alignment configuration
#define MSP_CURRENT_METER_CONFIG                                               \
  40 // out message: Get current meter configuration
#define MSP_SET_CURRENT_METER_CONFIG                                           \
  41                            // in message:  Set current meter configuration
...
#define MSP_CF_SERIAL_CONFIG                                                   \
  54 // out message: Get Cleanflight serial configuration
#define MSP_SET_CF_SERIAL_CONFIG                                               \
  55 // in message:  Set Cleanflight serial configuration
#define MSP_VOLTAGE_METER_CONFIG                                               \
  56 // out message: Get voltage meter configuration
#define MSP_SET_VOLTAGE_METER_CONFIG                                           \
  57                          // in message:  Set voltage meter configuration
...
#define MSP_RX_MAP                                                             \
  64 // out message: Get RX map (also returns number of channels total)
...
#define MSP_STATUS                                                             \
  101 // out message: Cycletime & errors_count & sensor present & box activation
...
#define MSP_RC_TUNING                                                          \
  111 // out message: RC rate, rc expo, rollpitch rate, yaw rate, dyn throttle
      // PID
#define MSP_PID 112      // out message: P I D coeff (9 are used currently)
#define MSP_BOXNAMES 116 // out message: The aux switch names
#define MSP_PIDNAMES 117 // out message: The PID names
#define MSP_WP                                                                 \
  118 // out message: Get a WP, WP# is in the payload, returns (WP#, lat, lon,
      // alt, flags) WP#0-home, WP#16-poshold
...
#define MSP_ESC_SENSOR_DATA                                                    \
  134 // out message: Extra ESC data from 32-Bit ESCs (Temperature, RPM)
#define MSP_GPS_RESCUE                                                         \
  135 // out message: GPS Rescue angle, returnAltitude, descentDistance,
      // groundSpeed, sanityChecks and minSats
#define MSP_GPS_RESCUE_PIDS                                                    \
  136 // out message: GPS Rescue throttleP and velocity PIDS + yaw P
...
#define MSP_MOTOR_TELEMETRY                                                    \
  139 // out message: Per-motor telemetry data (RPM, packet stats, ESC temp,
      // etc.)

// Simplified tuning commands (140-145)
#define MSP_SIMPLIFIED_TUNING                                                  \
  140 // out message: Get simplified tuning values and enabled state
#define MSP_SET_SIMPLIFIED_TUNING                                              \
  141 // in message:  Set simplified tuning positions and apply calculated
      // tuning
#define MSP_CALCULATE_SIMPLIFIED_PID                                           \
  142 // out message: Calculate PID values based on sliders without saving
#define MSP_CALCULATE_SIMPLIFIED_GYRO                                          \
  143 // out message: Calculate gyro filter values based on sliders without
      // saving
#define MSP_CALCULATE_SIMPLIFIED_DTERM                                         \
  144 // out message: Calculate D term filter values based on sliders without
      // saving
#define MSP_VALIDATE_SIMPLIFIED_TUNING                                         \
  145 // out message: Returns array of true/false showing which simplified
      // tuning groups match values

// Additional non-MultiWii commands (150-166)
#define MSP_STATUS_EX                                                          \
  150 // out message: Cycletime, errors_count, CPU load, sensor present etc
...
#define MSP_SET_TX_INFO                                                        \
  186 // in message:  Set runtime information from TX lua scripts
#define MSP_TX_INFO                                                            \
  187 // out message: Get runtime information for TX lua scripts
...
#define MSP_SET_RC_TUNING                                                      \
  204 // in message:  RC rate, rc expo, rollpitch rate, yaw rate, dyn throttle
      // PID, yaw expo
#define MSP_ACC_CALIBRATION                                                    \
  205 // in message:  No param - calibrate accelerometer
#define MSP_MAG_CALIBRATION                                                    \
  206                      // in message:  No param - calibrate magnetometer
...
#define MSP_SET_RESET_CURR_PID                                                 \
  219 // in message:  Reset current PID profile to defaults
#define MSP_SET_SENSOR_ALIGNMENT                                               \
  220 // in message:  Set the orientation of acc,gyro,mag
#define MSP_SET_LED_STRIP_MODECOLOR                                            \
  221 // in message:  Set LED strip mode_color settings
#define MSP_SET_MOTOR_CONFIG                                                   \
  222 // in message:  Motor configuration (min/max throttle, etc)
...
#define MSP_SET_VTXTABLE_POWERLEVEL                                            \
  228 // in message:  Set vtxTable powerLevel data

// Multiple MSP and special commands (230-255)
#define MSP_MULTIPLE_MSP                                                       \
  230 // out message: Request multiple MSPs in one request
...
#define MSP_SET_SIGNATURE                                                      \
  249 // in message:  Set the signature of the board and serial number
...
#define MSP_ASKARI 231 // Askari rate MSP command sends back attitude